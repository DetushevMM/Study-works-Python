...
# Рисунок виселицы, каждый элемент - следующая часть тела
hangman_parts = [
    '''
     -----
     |   |
...
def display_game_state(missed_letters, correct_letters, secret_word):
    print(hangman_parts[len(missed_letters)])
    print("Ошибочные буквы:", ' '.join(missed_letters))
    
    # Показать слово с отгаданными буквами и _ на месте остальных
    display_word = [letter if letter in correct_letters else '_' for letter in secret_word]
    print('Слово: ', ' '.join(display_word))
...
def play_game():
    ...
    missed_letters = []
    correct_letters = []
    print('Давайте играть в "Угадайка слов"!')
    ...
    while True:
        ...
        if guess in secret_word:
            ...
            if len(missed_letters) == len(hangman_parts) - 1:
                display_game_state(missed_letters, correct_letters, secret_word)
